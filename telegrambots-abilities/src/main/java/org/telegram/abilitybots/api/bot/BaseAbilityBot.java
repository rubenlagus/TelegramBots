...
public abstract class BaseAbilityBot extends DefaultAbsSender {
    ...
    private void registerAbilities() {
        try {
            ...
            Map<String, Ability> abilitiesMap = new HashMap<>();

            for (Method method : this.getClass().getMethods()) {
                if (method.getReturnType().equals(List.class)) {
                    List<Ability> abilityMethods = this.returnAbilities(method);
                    if (abilityMethods != null) {
                        abilityMethods.forEach(ability -> {
                            abilitiesMap.put(ability.name(), ability);
                        });
                    }
                }
            }

            abilities = new ImmutableMap.Builder()
                    .putAll(abilitiesMap)
                    .putAll(abilities)
                    .build();
    ...
    private List<Ability> returnAbilities(Method method) {
        try {
            return (List<Ability>) method.invoke(this);
        } catch (IllegalAccessException | InvocationTargetException e) {
            BotLogger.error("Could not add ability", TAG, e);
            throw propagate(e);
