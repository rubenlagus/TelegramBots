...
class TestExtension {
  ...
  private boolean hasAbilityNamed(String name) {
    return bot.getAbilities().values().stream().map(Ability::name).anyMatch(name::equals);
  }
  ...
  public static class ExtensionUsingBot extends AbilityBot {
    ...
    ExtensionUsingBot() {
      ...
      super(null, "", offlineInstance("testing"));
  ...
  public static class AbilityBotExtension implements AbilityExtension {
    ...
    public Ability abc() {
      return Ability.builder()
              ...
              .action(ctx -> {
                // https://github.com/rubenlagus/TelegramBots/issues/834
                extensionUser.getSilent().send("This is a test message.", ctx.chatId());
              })
              .build();
    }
  }
